...
class MADDPG:
    def __init__(self, params):
        ...
        self.ex_actors = [
            FCNetwork(
                (s.shape[0] + a.n, *params["network_size"], a.n), dropout=self.dropout
            ).to(device)
            for s, a in zip(state_sizes, action_sizes)
        ]

        self.actor_targets = [
            FCNetwork((s.shape[0], *params["network_size"], a.n), dropout=False).to(
                device
            )
            for s, a in zip(state_sizes, action_sizes)
        ]
        ...
        for target, source in zip(self.actor_targets, self.actors):
            target.hard_update(source)
        for target, source in zip(self.critic_targets, self.critics):
            target.hard_update(source)
    ...
    def select_actions(self, states, explore=False):
        ...
        """
        if not explore or self.expl_method == "dropout":
            actions = []
        ...
        assert self.expl_method == "cdropout"
...
def plot_rewards(r1, r2, eval_every):
    ...
    if len(y) > ma_length:
        plt.plot(x[len(x) - len(yMA) :], yMA, color="b", label="Coordinated Policies")

    plt.legend(title="MADDPG-AR with")
    # plt.title("Foraging-8x8-2p-2f-v0")
    plt.title("Evaluation of joint and decentralised policies during training")
    plt.savefig("cdropparal.pdf")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gym-env", default="Foraging-8x8-2p-4f-v0", type=str)

    parser.add_argument(
        "--expl-method", choices=["dropout", "cdropout"], default="cdropout", type=str
    )
